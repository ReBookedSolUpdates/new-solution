...
const EditUniform = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        setIsLoading(true);
        const item = await getBookById(id, 'uniform');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err?.message || "Failed to update uniform");
    } finally {
      ...
      <Layout>
        <div className="container mx-auto px-4 py-8 max-w-5xl">
          ...
          <div className="text-center">
            <h2 className="text-2xl font-semibold mb-4">Cannot Edit Uniform</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-8">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input name="title" value={formData.title} onChange={handleInputChange}
                    placeholder="e.g. Grey Boys Shorts" className={`${errors.title ? 'border-red-500' : ''} ${inputClass}`} />
                ...
                {/* Gender */}
                <div>
                  <Label className={`${labelClass} font-medium`}>Gender <span className="text-gray-400">(Optional)</span></Label>
                  <Select value={formData.gender || ''} onValueChange={(v) => handleSelectChange('gender', v)}>
                    <SelectTrigger><SelectValue placeholder="Select gender" /></SelectTrigger>
                    <SelectContent>
                      {['Male', 'Female', 'Unisex'].map(g => <SelectItem key={g} value={g}>{g}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                ...
                {/* Color */}
                <div>
                  <Label className={`${labelClass} font-medium`}>Color <span className="text-gray-400">(Optional)</span></Label>
                  <Input name="color" value={formData.color || ''} onChange={handleInputChange}
                    placeholder="e.g. White, Navy Blue, Grey" className={inputClass} />
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {/* Size */}
                <div>
                  <Label className={`${labelClass} font-medium`}>Size <span className="text-gray-400">(Optional)</span></Label>
                  <Select value={formData.size || ''} onValueChange={(v) => handleSelectChange('size', v)}>
                    <SelectTrigger><SelectValue placeholder="Select size" /></SelectTrigger>
                    <SelectContent>
                      {UNIFORM_SIZES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
            ...
            <Button type="submit" disabled={isSubmitting}
              ...
              {isSubmitting ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Updating Uniform...</>) : "👕 Update Uniform"}
            </Button>
          </form>
        </div>
      </div>
