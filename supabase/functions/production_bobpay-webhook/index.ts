...
const ACTIVE_API_URL = Deno.env.get('PRODUCTION_BOBPAY_API_URL');
const ACTIVE_API_TOKEN = Deno.env.get('PRODUCTION_BOBPAY_API_TOKEN');
const ACTIVE_PASSPHRASE = Deno.env.get('PRODUCTION_BOBPAY_PASSPHRASE');

console.log('[production-bobpay-webhook] Config check:', {
...
async function verifySignature(
  ...
  try {
    ...
    console.log('[production-bobpay-webhook] Calculated signature string:', signatureString);
    ...
    console.log('[production-bobpay-webhook] Calculated signature:', calculatedSignature);
    console.log('[production-bobpay-webhook] Received signature:', webhookData.signature);

    return calculatedSignature === webhookData.signature;
  } catch (error) {
    console.error('[production-bobpay-webhook] Signature verification error:', error);
    return false;
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[production-bobpay-webhook] Received webhook data:', JSON.stringify(webhookData, null, 2));

    // Signature verification — skip if empty (BobPay sandbox doesn't send signatures)
    if (webhookData.signature) {
      if (!ACTIVE_PASSPHRASE) {
        console.error('[production-bobpay-webhook] ACTIVE_PASSPHRASE not configured');
      ...
      if (!isValidSignature) {
        console.warn('[production-bobpay-webhook] Invalid signature detected');
        return new Response('Invalid signature', { status: 400 });
      }
      console.log('[production-bobpay-webhook] Signature verified successfully');
    }
    ...
    if (ACTIVE_API_URL && ACTIVE_API_TOKEN) {
      ...
      console.log('[production-bobpay-webhook] Validating with BobPay API at:', validationUrl);
      ...
      if (!validationResponse.ok) {
        ...
        console.error('[production-bobpay-webhook] BobPay validation failed:', valError);
        return new Response('Payment validation failed', { status: 400 });
      }
      console.log('[production-bobpay-webhook] BobPay API validation successful');
    }
